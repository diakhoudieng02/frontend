...
export function GoogleLoginButton({ mode }: GoogleLoginButtonProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast({
        title: 'Erreur',
        description: 'Impossible de lancer Google Sign-In',
        variant: 'destructive',
      });
      setLoading(false);
    }
  ...
  return (
    <button
      ...
      disabled={loading}
      className="glass-card w-full h-12 rounded-xl flex items-center justify-center gap-2.5 transition-all hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed group"
    >
      {loading ? (
        <>
